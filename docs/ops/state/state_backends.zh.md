...
<div class="alert alert-info" markdown="span">
  <strong>注意:</strong> 由于 RocksDB 是 Flink 默认提供的组件，如果在 `flink-conf.yaml` 中使用 `state.backend` 、 [checkpointing]({{ site.baseurl }}/zh/ops/config.html#checkpointing) 或 [RocksDB-specific]({{ site.baseurl }}/zh/ops/config.html#rocksdb-state-backend) 参数配置 State Backend，作业中没有使用任何 RocksDB 的代码，那么不需要添加此依赖。
</div>
