...
public class ResourceProfileTest {
	...
	public void testGet() {
		...
		assertEquals(1.0, rp.getCpuCores().getValue().doubleValue(), 0.000001);
		assertEquals(150, rp.getTotalMemory().getMebiBytes());
		assertEquals(100, rp.getOperatorsMemory().getMebiBytes());
		assertEquals(1.6, rp.getExtendedResources().get(GPUResource.NAME).getValue().doubleValue(), 0.000001);
	}
